...
const BOSS_DATA = [
    ...
    { id:'b02', name:'Brigand des Chemins',          act:1, actName:'Les Terres Sauvages', skill:'vigueur',      xpBase:3500,  timerDays:7,  img:'images/boss/acte1/bandit.jpg',         imgLegendaire:'images/boss/acte1/bandit_legendaire.jpg', emoji:'🗡️', lore:'Il rôde aux carrefours, attendant les voyageurs seuls.' },
    { id:'b03', name:'Ours des Cimes',               act:1, actName:'Les Terres Sauvages', skill:'agilite',      xpBase:3500,  timerDays:7,  img:'images/boss/acte1/ours.jpg',           imgLegendaire:'images/boss/acte1/ours_legendaire.jpg', emoji:'🐻', lore:'Gardien des hauteurs, personne ne passe sans son accord.' },
...
function gainSkillXP(skillKey, amount, label) {
    ...
    checkGlobalLevelUp();
    // V2.0: feed the creature — all XP nourishes totalXPInvested
    if (gameState.creature) {
        gameState.creature.totalXPInvested = (gameState.creature.totalXPInvested || 0) + boosted;
        gameState.creature.lastInteraction = new Date().toISOString();
        // Check for stage transition
        const newStage = computeCreatureStage();
        if (newStage > (gameState.creature.stage || 1)) {
            gameState.creature.stage = newStage;
            if (newStage >= 3 && !gameState.creature.branch) {
                gameState.creature.branch = computeCreatureBranch();
                gameState.creature.branchLockedAt = new Date().toISOString();
                // Reveal branch modal (deferred to avoid blocking XP flow)
                setTimeout(() => showBranchRevealModal(), 300);
            }
        }
    }
}
...
function migrateGameState() {
    ...
    // V2.0: transition from 48h rest to "next midnight" — snap restUntil if too far in future
    if (gameState.campaign?.restUntil) {
        const nextMidnight = getBossAvailableAfterVictory();
        if (new Date(gameState.campaign.restUntil) > nextMidnight) {
            gameState.campaign.restUntil = nextMidnight.toISOString();
        }
    }
    ...
    // V2.0: creature system — the living companion that replaces the avatar
    if (!gameState.creature) gameState.creature = {
        type: 'wolf', name: 'Loup',
        totalXPInvested: 0,
        stage: 1,
...
// completed may no longer be valid. Revoke it: remove its XP and its completedId,
// so it becomes completable again and no XP is duplicated.
function revokeInvalidQuests() {
    if (!gameState.quests) return;
    const ids = gameState.quests.completedIds || [];
    ...
    allQuests.forEach(quest => {
        ...
        if (wasMarked && !isQuestComplete(quest)) {
            ...
            changed = true;
            const cfg = SKILL_CONFIG[skillKey];
            toast(`↩️ Quête "${quest.title}" annulée — ${quest.xp} XP retiré`, 'info');
        }
    });
...
function toggleHabit(habitId) {
    ...
    if (habit.completed && !wasCompleted) {
        ...
        // V2.0: streak bonus capped at +100% (was unlimited — caused exponential XP at high streaks)
        habit.streak = computeHabitStreak(habit);
        const streakBonus = Math.min(1.0, (habit.streak||0) * 0.05); // max +100% at streak 20+
    ...
    saveGameState();
    // V10.0c: if we just unchecked, revoke quests that are no longer valid
    if (wasCompleted && !habit.completed) revokeInvalidQuests();
    checkQuestProgress();
    renderUI();
}
    ...
    overlay.innerHTML = `
        <div class="modal-box" style="border-color:rgba(245,200,66,0.2);">
            ...
            <button class="btn-gold" style="width:100%;margin-top:8px;"
                onclick="document.getElementById('settings-overlay').remove();confirmResetV2();">
                🚀 Démarrer Ascend V2.0
            </button>
...
// V2.0: Combat ends at 23h59 of the 14th day after starting (simple, predictable).
// Example: start Monday 8am → deadline Monday+14 at 23:59.
...
// V2.0: After victory, next boss is available at 0h00 of the NEXT day.
// Victoire 08:00 → wait 16h; victoire 23:59 → wait 1min. Simple and fair.
function getBossAvailableAfterVictory() {
...
// Kept for reference but no longer used in V2.0
...
function defeatBoss(boss) {
    ...
    _displayBossId = null;
    _displayAct = null; // V10.0c: snap back to current act after victory
    if (!isRefight) {
...
let _displayAct = null; // V10.0c: which act's boss list is shown (null = act of current boss)
function selectDisplayBoss(bossId) {
    _displayBossId = bossId;
    const b = BOSS_DATA.find(x => x.id === bossId);
    if (b) _displayAct = b.act;
    renderCampagneContent();
}
// V10.0c: browse acts to re-fight old bosses (e.g. for all-Legendary).
// Picks the first defeated/available boss of that act as the displayed one.
function navigateDisplayAct(dir) {
    const acts = [...new Set(BOSS_DATA.map(b => b.act))].sort((a,b)=>a-b);
    const cur = _displayActCurrent();
    const idx = acts.indexOf(cur);
    const next = acts[Math.min(acts.length-1, Math.max(0, idx + dir))];
    _displayAct = next;
    // Show the first boss of that act that the player can actually look at
    const actBosses = BOSS_DATA.filter(b => b.act === next);
    const c = gameState.campaign;
    const preferred = actBosses.find(b => c && c.currentBossId === b.id)
                   || actBosses.find(b => isBossUnlocked(b.id) || (c && c.bossDefeated.includes(b.id)))
                   || actBosses[0];
    _displayBossId = preferred ? preferred.id : null;
    renderCampagneContent();
}
function _displayActCurrent() {
    if (_displayAct) return _displayAct;
    const avail = getCurrentAvailableBoss();
    const displayBoss = _displayBossId ? BOSS_DATA.find(b=>b.id===_displayBossId) : avail;
    return displayBoss ? displayBoss.act : 1;
}
...
function buildBossTab() {
    ...
    const avail = getCurrentAvailableBoss();
    const displayBoss = _displayBossId ? BOSS_DATA.find(b=>b.id===_displayBossId) : avail;
    if (!displayBoss) return `<div class="empty-state" style="padding-top:60px"><span class="ei">🏆</span><p>Épreuves terminées !</p></div>`;
    ...
    if (isFighting && c.bossStartDate) {
        const endDate = getBossEndDate(c.bossStartDate);  // J+14 at 23:59
        const remMs = endDate - getNow();
        const rem = Math.max(0, remMs / 86400000);
        const days = Math.floor(rem);
        const hours = Math.floor((rem - days) * 24);
        // V2.0: show X days / 14 (not "X days / N days until Monday")
        const elapsed = Math.floor((getNow() - new Date(c.bossStartDate)) / 86400000);
        const dayUsed = Math.min(14, elapsed + 1);
        timerHtml = `<div class="boss-timer-row"><span class="boss-timer-label">Temps restant :</span><span class="boss-timer-val">${days}j ${hours}h</span><span class="boss-timer-max">· jour ${dayUsed} / 14</span></div>`;
    }
    // Status
    let statusHtml = '';
    if (isResting) {
...
/* ═══════════════════════════════════════════
   V2.0 — CREATURE SYSTEM
   The living companion that replaces the avatar.
   Grows from all XP earned. Never regresses.
   ═══════════════════════════════════════════ */
...
const CREATURE_BRANCH_NAMES = {
    power:  { name: 'Loup de Guerre',     icon: '🔴', color: '#ff4444' },
    arcane: { name: 'Loup des Prophètes', icon: '🔵', color: '#4ecdc4' },
    legion: { name: 'Loup Légionnaire',   icon: '🟡', color: '#f5c842' },
    spirit: { name: 'Loup Esprit',        icon: '🟣', color: '#9f7aea' },
    shadow: { name: 'Seigneur des Ombres',icon: '🔵', color: '#3d8ef0' },
    primal: { name: "L'Ancien",           icon: '⚪', color: '#ffffff' }
};

function computeCreatureStage() {
    const xp = gameState.creature?.totalXPInvested || 0;
    let stage = 1;
...
function getCreatureState() {
    ...
    // Check if any action done today
    const todayStr = getLocalDateStr(getNow());
    const hasActionToday = (getLocalDateStr(new Date(c.lastInteraction)) === todayStr);
    if (hasActionToday) return 'radiant';
    if (hoursSince < 48) return 'waiting';
    return 'sleeping';
}

function getCreatureStageProgress() {
    const xp = gameState.creature?.totalXPInvested || 0;
    const stage = computeCreatureStage();
    const current = CREATURE_STAGE_THRESHOLDS.find(t => t.stage === stage);
    const next = CREATURE_STAGE_THRESHOLDS.find(t => t.stage === stage + 1);
    if (!next) return { pct: 100, xpInStage: xp - current.xp, xpNeeded: 0, label: 'MAX' };
    const inStage = xp - current.xp;
    const needed = next.xp - current.xp;
    const pct = Math.min(100, Math.floor(inStage / needed * 100));
    return { pct, xpInStage: inStage, xpNeeded: needed,
...
function renderCreatureWidget() {
    const c = gameState.creature;
    if (!c) return '';
    const stage = computeCreatureStage();
    const state = getCreatureState();
    const progress = getCreatureStageProgress();
    const stateLabels = {
        radiant: `<span style="color:#7dcc7d">✨ ${c.name} est avec toi</span>`,
        waiting: `<span style="color:var(--text-dim)">Il t'attend...</span>`,
        sleeping: `<span style="color:#666">Il repose. Il sera là à ton retour.</span>`
    };
    const stageName = ['','Louveteau','Jeune Loup','Loup Adulte','Forme Mythique'][stage];
    const branchBadge = c.branch
        ? `<span class="creature-branch-badge" style="color:${CREATURE_BRANCH_NAMES[c.branch]?.color}">${CREATURE_BRANCH_NAMES[c.branch]?.icon} ${CREATURE_BRANCH_NAMES[c.branch]?.name}</span>`
        : '';
    const xpLabel = stage < 4 ? progress.label : `${(c.totalXPInvested||0).toLocaleString()} XP total`;
    return `
    <div class="creature-widget">
        <div class="creature-visual" onclick="openCreatureProfile()">${creatureSVG()}</div>
        <div class="creature-name-row">
            <span class="creature-name" onclick="openRenameCreature()">${escapeHtml(c.name)} ✎</span>
            ${branchBadge}
        </div>
        <div class="creature-state-label">${stateLabels[state] || ''}</div>
        <div class="creature-stage-label">${stageName}</div>
        <div class="creature-progress-bar-bg">
            <div class="creature-progress-bar-fill" style="width:${progress.pct}%;background:${c.branch ? CREATURE_BRANCH_NAMES[c.branch]?.color : '#f5c842'}"></div>
        </div>
        <div class="creature-progress-label">${xpLabel}</div>
    </div>`;
}

function openRenameCreature() {
    const c = gameState.creature;
    if (!c) return;
    document.querySelectorAll('#rename-creature-modal').forEach(m => m.remove());
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay'; overlay.id = 'rename-creature-modal';
    overlay.innerHTML = `
        <div class="modal-box">
            <div class="modal-title gold">✎ Renommer ton loup</div>
            <div class="modal-body">
                <input type="text" id="creature-name-input" class="modal-input"
                       value="${escapeHtml(c.name)}" maxlength="24" placeholder="Nom de ton loup" />
            </div>
            <div class="modal-actions">
                <button class="btn-cancel" onclick="document.getElementById('rename-creature-modal').remove()">Annuler</button>
                <button class="btn-gold" onclick="commitRenameCreature()">Valider</button>
            </div>
        </div>`;
    document.body.appendChild(overlay);
    overlay.addEventListener('click', e => { if (e.target === overlay) overlay.remove(); });
    setTimeout(() => document.getElementById('creature-name-input')?.select(), 50);
}

function commitRenameCreature() {
    const name = document.getElementById('creature-name-input')?.value.trim();
    if (!name) return;
    gameState.creature.name = name;
    saveGameState();
...
function openCreatureProfile() {
    // Navigates to the Profil tab creature section
    switchTab('profil');
}

// ── RESET V2.0 ──────────────────────────────────────────

function confirmResetV2() {
    showModal({
        type: 'danger',
        title: '🚀 Démarrer Ascend V2.0 ?',
        body: `<div style="font-size:0.82rem;line-height:1.6">
            <div style="color:#7dcc7d;margin-bottom:8px">✅ Ce qui est conservé :</div>
            <div>Journal · Humeurs · Habitudes (et leurs streaks)</div>
            <div style="color:#ff6b6b;margin-top:8px;margin-bottom:8px">🗑 Ce qui est effacé :</div>
            <div>XP de compétences · Campagne · Tâches · Logs de sessions · Stats · Titres</div>
            <div style="margin-top:10px;color:var(--text-dim);font-style:italic">Ton loup naît ici. Tout ce que tu feras à partir de maintenant le façonnera.</div>
        </div>`,
        confirmLabel: 'Commencer V2.0',
        onConfirm: () => confirmResetV2Step2()
    });
}

function confirmResetV2Step2() {
    // Second confirmation — final
    showModal({
        type: 'danger',
        title: '⚠️ Dernière confirmation',
        body: 'Cette action est irréversible. Pense à exporter une sauvegarde si tu veux garder une trace de ton parcours V1.',
        confirmLabel: '✅ Oui, je démarre V2.0',
        onConfirm: () => executeResetV2()
    });
}

function executeResetV2() {
    const old = gameState;
    // Build the new state
    const newState = {
        _schemaVersion: 'V2.0',
        username: old.username || 'Hero',
        skills: {
            endurance:  { level: 1, currentXP: 0, totalXP: 0 },
            sagesse:    { level: 1, currentXP: 0, totalXP: 0 },
            discipline: { level: 1, currentXP: 0, totalXP: 0 },
        ...
        // Keep habits list + streaks, only reset completion state for today
        habits: (old.habits || []).map(h => ({
            ...h, completed: false
            // Keep: streak, history, name, frequency, category, etc.
        })),
        // Keep sessions structure, clear logs
        sessions: (old.sessions || []).map(s => ({
            ...s, logs: [], streak: 0, partialCredit: []
        })),
        // PRESERVE journal entirely
        journal: old.journal || {
            pinHash: null, entries: {}, streak: 0, longestStreak: 0,
            lastEntryDate: null, xpAwardedDates: []
        },
        // PRESERVE moods entirely
        moods: old.moods || [],
        tasks: [], taskStreak: 0, longestTaskStreak: 0,
        taskCompletionDates: [], tasksXPAwardedByDate: {},
        stats: { totalHabitsCompleted: 0, perfectDays: 0,
                 perfectStreak: 0, longestPerfectStreak: 0, longestStreak: 0 },
        xpHistory: [],
        ...
        activeTimer: null,
        creature: {
            type: 'wolf', name: old.creature?.name || 'Loup',
            totalXPInvested: 0,
            stage: 1, branch: null, branchLockedAt: null,
            shards: { wild:0, stone:0, flame:0, abyss:0, cosmic:0 },
            cosmetics: [],
            activatedAt: new Date().toISOString(),
            lastInteraction: null, dailyPopupSeenDate: null,
            popupEnabled: old.creature?.popupEnabled !== false
        },
        lastResetDate: getLocalDateStr(new Date())
    };
    gameState = newState;
    migrateGameState(); // ensures all fields are present
    initCampaign();
    saveGameState();
    toast('🐺 Bienvenue dans Ascend V2.0 ! Ton loup t\'attend.', 'success');
    renderUI();
}
